...
-- Set default values for new columns in existing trades (only if trades table exists)
DO $$
BEGIN
...
-- Daily performance view (PostgreSQL compatible)
...
-- Symbol performance view (PostgreSQL compatible)
...
-- ====
-- 11. VERIFICATION QUERIES (PostgreSQL compatible)
-- ====
...
BEGIN
    FOR rec IN 
        ...
        UNION ALL SELECT 'performance_metrics'
        UNION ALL SELECT 'ml_predictions'
        UNION ALL SELECT 'risk_management'
        UNION ALL SELECT 'webhook_logs'
        UNION ALL SELECT 'bot_status'
    LOOP
