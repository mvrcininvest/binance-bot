...
def main():
    ...
    ap.add_argument("--backup", action="store_true", help="Write .bak backup before overwrite")
    ap.add_argument(
        "--no-newlines-normalize", action="store_true", help="Do not normalize newlines"
    )
    ap.add_argument("--keep-bom", action="store_true", help="Do not strip UTF-8 BOM if present")
    args = ap.parse_args()
